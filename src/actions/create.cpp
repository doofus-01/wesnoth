...
std::vector<unit_const_ptr > get_recalls(int side, const map_location &recall_loc)
{
	...
	if ( find_it != resources::units->end() ) {
		...
		else if ( find_it->is_visible_to_team(resources::gameboard->get_team(side), resources::gameboard->map(), false) )
		{
	...
	if ( !leader_in_place )
	{
		...
		for (const unit_const_ptr & recall : resources::gameboard->get_team(side).recall_list())
		{
...
namespace { // Helpers for check_recall_location()
		...
		const map_location & preferred, map_location & alternative)
	{
		...
		const team& recall_team = resources::gameboard->get_team(recaller.side());
                                     ...
                                     const std::string& unit_type)
{
	...
	if ( util::contains(resources::gameboard->get_team(side).recruits(), unit_type) )
		check_type.clear();
		...
		const map_location &recruit_loc, const map_location &leader_loc)
	{
		...
		for ( unit_itor = units.begin(); unit_itor != units.end(); ++unit_itor ) {
			if (resources::gameboard->get_team(unit_itor->side()).is_enemy(new_unit.side()) &&
				unit_itor->is_visible_to_team(resources::gameboard->get_team(new_unit.side()), *map, false)) {
